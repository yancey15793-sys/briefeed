...
- **Podcast — `#podcastPage` à 2 onglets** (onglets `.pc-tab` Abonné/Découvrir existants, `pcSetTab` rend les panes ; réf. The Athletic + Apple/Spotify) : bouton `.sbar-podcast` (Smart Bar) → `openPodcast('abonne')` (`openPodListen` est un alias). **Abonné** (`#pcPaneAbonne` ← `_plBuildAbos`) : Continuer l'écoute (`.pl-cont`, suivi `bf_pl_pos` dans `timeupdate`/`pause`, reprise via `loadedmetadata`), Derniers épisodes (`.pl-ep`), Mes podcasts (`.pl-pod` sources + badge), Lire à voix haute (TTS fr-FR). **Découvrir** (`#pcPaneDecouvrir` ← `_plBuildDiscover`) : recherche annuaire **Apple Podcasts** (`_plItunes` : fetch direct + proxies CORS) + abonnement 1-tap (`_plSubscribe` → dossier « Podcasts »). Mini-player `#plPlayer` **global** (position:fixed, z-index 460, lecture persistante hors page), route audio/TTS (`_plMode`). `_plSyncPlaying` cible `#pcPaneAbonne`. **`#podListenPage` + `.pl-tabs` supprimés (doublon).** Suite lot 2 : tendances/catégories/hero via iTunes ; galerie thèmes `PC_THEMES` mise de côté.
