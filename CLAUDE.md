...
- **Podcast — page à 2 onglets** (refonte en cours, réf. The Athletic + Apple/Spotify) : bouton `.sbar-podcast` (Smart Bar) = seule entrée → `#podListenPage` (`openPodListen`), retour `#plBack` « Accueil ». Sélecteur `.pl-tabs` (`_plTab`:`abos`/`discover`). **Abonnements** (`_plBuildAbos`) : Continuer l'écoute (carrousel `.pl-cont`, suivi position `bf_pl_pos` sauvé dans `timeupdate`/`pause`, reprise via `loadedmetadata`), Derniers épisodes (liste `.pl-ep`), Mes podcasts (carrousel `.pl-pod` des sources audio + badge), Lire à voix haute (TTS fr-FR). **Découvrir** (`_plBuildDiscover`) : provisoire → **API Apple Podcasts (iTunes Search, gratuite) prévue au lot 2** (tendances/catégories/recherche/abonnement). Mini-player route audio/TTS (`_plMode`). Fns : `_plMyPods`/`_plContinue`/`_plSavePos`/`_plSetTab`/`_plPodCard`/`_plContCard`. Galerie thèmes `#podcastPage` (`openPodcast`) toujours là (bouton « Parcourir les thèmes »).
